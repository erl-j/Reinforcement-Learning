	...
	if s == 900:
		...
		if p == m or p == 28:
			...
			for m_new in MINOTAUR_ACCESS[m]:
				tp[p_new + 30 * m_new] = (1 / n_m_new)*(1-P_DEATH)
			tp[900]=P_DEATH;
	return tp
...
def backward_induction(stps,rewards,T):
	...
	for t in range(T-2, 0, -1):
		...
		policy[:,t]=u_a;
		if t%10:
			print("Bellman at t = {}     ".format(t),end="\r");
		# print(u_star)
	return policy
...
total_win_time=0;
for i in range(N):
	isWin,t_end=try_policy(policy,T);
	if isWin:
		wins+=1;
		total_win_time+=t_end;
mean_win_time=total_win_time/wins;
